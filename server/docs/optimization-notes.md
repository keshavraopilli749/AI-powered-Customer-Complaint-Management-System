...
1. **Pagination & Virtualization**: The Complaints table currently fetches all records. Implement offset/limit pagination on the server and use `react-window` on the client.
...
2. **Celery / Background Tasks**: The LangGraph AI pipeline currently runs synchronously within the FastAPI request cycle. For large PDF documents, this should be offloaded to a Celery worker, with the client polling for status updates via WebSockets.
